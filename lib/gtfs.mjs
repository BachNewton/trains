 ...
 */
export function loadFeed(dir, { today = todayStamp() } = {}) {
  const t0 = Date.now();
  ...
  const nodeOf = new Map(); // stop_id -> node id (parent station if any)
  const stations = new Map(); // node id -> {id, name, lat, lon}
  for (const s of stopRows) {
    const node = s.parent_station && s.parent_station.length ? s.parent_station : s.stop_id;
    nodeOf.set(s.stop_id, node);
  }
  for (const s of stopRows) {
    const node = nodeOf.get(s.stop_id);
    // prefer the parent row itself for the canonical name/coords
    if (!stations.has(node) || s.stop_id === node) {
      stations.set(node, {
        id: node,
        name: s.stop_name,
  ...
  for (const [tripId, stops] of byTrip) {
    ...
    for (let i = 0; i < seq.length; i++) {
      for (let j = i + 1; j < seq.length; j++) {
        ...
        let e = nbrs.get(to);
        if (!e) { e = { minDuration: Infinity, routes: new Set(), start: win.start, end: win.end }; nbrs.set(to, e); }
        const dep = seq[i][1];
        const arr = seq[j][2];
